...
while iterations < #move.axes ; Loop over all axes
    if {(move.axes[iterations].letter ^ "") == "W"} ; A W-axis is defined, so we have a motor-driven cleaning station.
        ...
        G1 W26 F15000 ; Move W to 26mm at 15000 mm/min to extend the cleaning station pliers to surround the nozzle (Wclean)
		G4 P125 ; Dwell for 125 ms
        M400 ; Wait for current moves to finish
        M42 P1 S1 ; Close pliers
        G4 P125 ; Dwell for 125 ms
        G1 W18 F15000 ; Move W to 18mm at 15000 mm/min to retract the cleaning station pliers to pull the filament button from the nozzle (Wgrab)
        G28 W ; Home W
