...
M558 K0 F150
G38.2 X{move.axes[0].userPosition + 6} P0 ; Adjust the + n amount to the travel limit (expected hole diameter is an upper bound)
M400
G60 S2                                     ; Save +X position to slot 2
G0 X0 R1 F300                              ; Return X to start position (disengage the probe) (slot 1)
G38.2 X{move.axes[0].userPosition - 6} P0 ; Ajust the - n amount to the travel limit
G0 X{move.axes[0].machinePosition + ((state.restorePoints[2].coords[0] - move.axes[0].machinePosition) / 2.0)} ; Move to pretty good X-value of hole center

; Probe in Y direction ("fine")
G38.2 Y{move.axes[1].userPosition + 8} P0 ; Adjust the + n amount to the travel limit
M400
...
G0 Y{move.axes[1].machinePosition + ((state.restorePoints[3].coords[1] - move.axes[1].machinePosition) / 2.0)} F300; Move to very good Y-value of hole center

M400
G60 S1                                     ; Save "pretty good X" and "very good Y" position to slot 1
M558 K0 F100
; Probe in X direction ("fine")
G38.2 X{move.axes[0].userPosition + 8} P0 ; Adjust the + n amount to the travel limit
M400
G60 S4                                     ; Save +X position to slot 4
G0 X0 R1 F300                              ; Return X to pretty good center position (disengage the probe) (slot 1)
G38.2 X{move.axes[0].userPosition - 8} P0 ; Ajust the - n amount to the travel limit
; M400
...
G0 X{move.axes[0].machinePosition + ((state.restorePoints[4].coords[0] - move.axes[0].machinePosition) / 2.0)} F300; Move to very good X-value of hole center
M558 K0 F150