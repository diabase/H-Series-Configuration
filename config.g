...
while(iterations < 5)
    if {"exists(global.E"^iterations^"Drive)"} && {"exists(global.E"^iterations^"Direction)"}
        M569 P{global.E{iterations}Drive} S{global.E{iterations}Direction} ;  Set motor driver direction. Line 37: E (Extruder)
while(iterations < 5)
    if exists(global.FA{iterations}Drive) && exists(global.FA{iterations}Direction)
...
; Probes
M558 P8 C"1.io0.in" H2 F150 T10000; ; Set Z probe type for Probe 0 (Tool 10). It's a normally closed switch between the "zprobe.in" pin and ground using the internal pullup resistor, 2mm dive height, 150mm/min probing speed, and 10000 mm/min travel speed between probe points
M558 K1 P8 C"1.io5.in" I0 F200 T10000 ; Set Z probe type for Probe 1 (touchoff plate). It's an unfiltered normally closed switch between the "1.io5.in" pin and ground, non-inverted probe reading, 200mm/min probing speed, and 10000 mm/min travel speed between probe points
