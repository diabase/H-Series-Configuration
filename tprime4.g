...
if state.currentTool == -1
    ...
    G92 U{tools[{state.currentTool}].offsets[3]-tools[4].offsets[3]} ; We don't want to move the turret before the first priming rotation, so we falsely set the current turret position to the expected position for Tool 1 which causes tpre-universal.g to skip the Z-hop and turret rotation.
