...
G38.2 X{move.axes[0].userPosition + 10} P1 ; Adjust the + n amount to the travel limit (expected hole diameter is an upper bound)
M400
G60 S2                                     ; Save +X position to slot 2
G0 X0 R1 F300                              ; Return X to start position (disengage the probe) (slot 1)
G38.2 X{move.axes[0].userPosition - 10} P1 ; Ajust the - n amount to the travel limit
G0 X{move.axes[0].userPosition + ((state.restorePoints[2].coords[0] - move.axes[0].userPosition) / 2.0)} ; Move to pretty good X-value of hole center

; Probe in Y direction ("fine")
G38.2 Y{move.axes[1].userPosition + 10} P1 ; Adjust the + n amount to the travel limit
M400
...
G0 Y{move.axes[1].userPosition + ((state.restorePoints[3].coords[1] - move.axes[1].userPosition) / 2.0)} F300; Move to very good Y-value of hole center

M400
G60 S1                                     ; Save "pretty good X" and "very good Y" position to slot 1
M558 K1 F100
; Probe in X direction ("fine")
G38.2 X{move.axes[0].userPosition + 10} P1 ; Adjust the + n amount to the travel limit
M400
...
G0 X{move.axes[0].userPosition + ((state.restorePoints[4].coords[0] - move.axes[0].userPosition) / 2.0)} F300; Move to very good X-value of hole center
M558 K1 F200
