...
if {global.machineModel} == "H4" || {global.machineModel} == "H5A"
    ...
    if move.axes[var.uAxisIndex].machinePosition != -tools[state.nextTool].offsets[var.uAxisIndex]
        G91 ; Relative Positioning

        if move.axes[var.zAxisIndex].machinePosition < {move.axes[var.zAxisIndex].max - 40}  ; If we have enough room for a normal tool change Z-hop, do it.
            G1 Z40 F10000 ; Move Z +40mm at 10000 mm/min

        elif move.axes[var.zAxisIndex].machinePosition > {move.axes[var.zAxisIndex].max -40} ; If we don't have enough room, move as high as we can.
        ...
        elif move.axes[var.zAxisIndex].machinePosition == {move.axes[var.zAxisIndex].max -40} ; If we're already at the safe z position for a tool change...
            M118 S{"tpre-universal.g: Already at safe z position for tool change."} L3     ; ... do nothing
        M400
        M98 P"unlock_turret.g" ; Call unlock_turret.g
        G90 ; Absolute Positioning
        G1 U{-tools[state.nextTool].offsets[var.uAxisIndex]} F10000 ; Rotate turret to new tool
    ...
    ; Only perform machine moves if we need to change the turret position
    if {move.axes[var.uAxisIndex].machinePosition != -tools[state.nextTool].offsets[var.uAxisIndex] || {state.nextTool} >= 11}
        M400                                                                    ; Wait for all moves to finish
        if move.axes[var.zAxisIndex].machinePosition < {move.axes[var.zAxisIndex].max - 40}               ; If we have enough room for a normal tool change Z-hop, do it.
            ...
            M574 Z2 S1 P{global.zSwitchPin}                                     ; Configure Z endstop position at high end, it's a microswitch on pin defined in defaultparameters.g
            G90                                                                 ; Absolute positioning
            G53 G1 Z{move.axes[var.zAxisIndex].max} F10000                                   ; Move to ZMax
            M400                                                                ; Wait for all moves to finish

        if {{state.nextTool} >= 11}
            G90                                                                                                             ; Absolute Positioning
            G1 B{-tools[state.nextTool].offsets[var.bAxisIndex]} F30000                                                                ; Move tool changer to position for upcoming tool
            ...
            if {abs({move.axes[var.uAxisIndex].machinePosition}-{{-tools[state.nextTool].offsets[var.uAxisIndex]}+180}) > 1.0}                       ; If the turret isn't already pointing the spindle within 1 degree of the tool changer...
                M118 S{"Current turret position is " ^ move.axes[var.uAxisIndex].machinePosition ^ " and we need it to be " ^ {{-tools[state.nextTool].offsets[var.uAxisIndex]}+180}} L3
                M400
                M98 P"unlock_turret.g"                                                                                      ; Call unlock_turret.g
                G1 U{{-tools[state.nextTool].offsets[var.uAxisIndex]}+180} F10000                                                      ; Rotate turret to point the spindle at the tool changer
                M400
            ...
            G53 G1 Z{{move.axes[var.zAxisIndex].max}+{global.tCOvertravelPutTool}-0.5} H2 F3000                                          ; Move Z beyond ZMax ignoring endstops
            M400                                                                                                            ; Wait for all moves to finish
            ...
            M118 S{"Normal Max Z Motor Current is "^move.axes[var.zAxisIndex].current^ "mA. Reducing by 50%."} L3
            var zMotorCurrent = move.axes[var.zAxisIndex].current
            M906 Z{var.zMotorCurrent * 0.5}
            M118 S{"Max z-motor current is now "^move.axes[var.zAxisIndex].current^ "mA. Attempting downward move."} L3
            G1 Z{move.axes[var.zAxisIndex].max} H2 F10000
            M906 Z{var.zMotorCurrent}
            M118 S{"Attempted move finished. Max z-motor current is now "^move.axes[var.zAxisIndex].current^ "mA."} L3
        ...
        if global.dontRotate != 1
            ...
            if {{state.nextTool} >= 11}
                ...
                G53 G1 U{-tools[state.nextTool].offsets[var.uAxisIndex]} Z{move.axes[var.zAxisIndex].max} F10000                                        ; Rotate turret to active position for new tool and move up to ZMax
            else
                M118 S{"Current U position is " ^ move.axes[var.uAxisIndex].machinePosition ^ ", but we want " ^ {-tools[state.nextTool].offsets[var.uAxisIndex]}} L3
                M118 S{"tpre-universal.g: Moving to U position for next tool"} L3
                G53 G1 U{-tools[state.nextTool].offsets[var.uAxisIndex]} F10000                                                            ; Rotate turret to active position for new tool
            G4 P20 ; Dwell for 20 ms
            M400                                                                                                            ; Wait for any current moves to finish
            M98 P"lock_turret.g" ; Call lock_turret.g
